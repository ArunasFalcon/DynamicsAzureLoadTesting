param(
    [string]$JMXProjectFile,
    [string]$JTLorXMLRecordingFile
)

$ErrorActionPreference = 'Stop'

if ([string]::IsNullOrEmpty($JMXProjectFile) -or [string]::IsNullOrEmpty($JTLorXMLRecordingFile))
{
    ...
    #file paths already provided, run non-interactively
    $JMXFilePath = (Get-Item -Path $JMXProjectFile).FullName
    $JTLFilePath = (Get-Item -Path $JTLorXMLRecordingFile).FullName
}
...
$RequestIds=@()
$ResultsFile.testResults.httpSample | Select-Object lb |
  ForEach-Object {
             $RequestIds += $_."lb"       
                 }
